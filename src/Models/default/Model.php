...
class Model{
    ...
    public function createUnique($arg, $table = false, $pass = true){//create a new element using the $arg. ex : ["name" => "Test", "email" => "test@test.com"]
        $keys = array_keys($arg);
        if($table == false){
            $table = $this->table;
        }
    ...
    public function lastinsertid($c = 'id', $table = false){//return the last inserted id
        if($table == false){ $table = $this->table; }
        $obj = $this->query("SELECT $c FROM $table ORDER BY $c DESC LIMIT 1")[0];
        $arr = (array) $obj;
        return $arr["$c"];
    }

    public function count($tag = '*', $r = false){
        $this->params['SELECT'] = 'COUNT('.$tag.') as count';
        if($r){
            $data = $this->run();
            if(empty($data)){
                return 0;
