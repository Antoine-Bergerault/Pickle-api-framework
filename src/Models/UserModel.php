...
class UserModel extends Model{
    ...
    public function get_user($arg){//get the user info only if the pass are correct (used to connect users)
        ...
        if($usr != false){
            if(!isset($arg['pass']) || (isset($usr->pass) && password_verify($arg['pass'], $usr->pass))){
                return $usr;
            }
        }
        return false;
    ...
    public function getStats(array $categories){
        if(\Pickle\Engine\App::is_connected()){
            $data = [];
            foreach($categories as $category){
                $data[$category->name] = null;
            }
            $sql = "SELECT categories.name as category, categories.id as category_id, AVG(results.percentage) as `average` FROM categories INNER JOIN quizz ON quizz.category_id = categories.id INNER JOIN results ON results.quizz_id = quizz.id WHERE results.user_id = ? GROUP BY categories.id;";
            $sqlData = [\Pickle\Engine\App::getid()];
            $result = $this->query($sql, $sqlData);
            foreach($result as $r){
                $data[$r->category] = $r->average;
            }
            return $data;
        }
        return [];
    ...
    public function getStatsFromName(array $categories, string $name){
        $data = [];
        foreach($categories as $category){
            $data[$category->name] = null;
        }
        $sql = "SELECT categories.name as category, categories.id as category_id, AVG(results.percentage) as `average` FROM categories INNER JOIN quizz ON quizz.category_id = categories.id INNER JOIN results ON results.quizz_id = quizz.id INNER JOIN users ON users.id = results.user_id WHERE users.name = ? GROUP BY categories.id;";
        $sqlData = [$name];
        $result = $this->query($sql, $sqlData);
        if(empty($result)){
            return [];
